...
const controls = [
  { code: "KPC-LIC-001", name: "Bases de licitud", domain_id: "d1", verification_criteria: ["¿Existe registro de bases?", "¿Se recoge consentimiento?"] },
  { code: "KPC-XXX-000", name: "Sin criterios", domain_id: "d2", verification_criteria: [] },
];
...
describe("buildComplianceQuestions", () => {
  it("emits one question group per control with criteria", () => {
    ...
    expect(q[0]).toMatchObject({ controlCode: "KPC-LIC-001", criteria: controls[0].verification_criteria });
  });
});
```
